...
class DetectionLoader:
    ...
    def start(self):
        t = Thread(target=self.update(), args=(self.data_loader))
        t.daemon = True
        t.start()
    ...
    def update(self):
        while True:
            frame = self.data_loader.getitem()
            ...
            for n in range(len(personwiseKeypoints)):
                ...
                if self.totalframecount != 0 and self.totalframecount % 10 == 0:
                    if ((self.new_neck[n] - self.old_neck[n]) > self.subject_height[n]*self.fall_ratio) and self.new_neck[n] != -1 and self.old_neck[n] != -1 and self.subject_height[n] != -1:
                        self.fallcount += 1
                        cv2.imwrite('/home/htxsns/projects/MobileNetV2-PoseEstimation/output/img/'+str(self.fallcount)+'.jpg', frameClone)
